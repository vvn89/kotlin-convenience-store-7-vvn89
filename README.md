...
- 고객으로부터 상품명과 수량을 입력받는다.
- 포스기로부터 안내를 전달받아 고객에게 안내에 대한 요청을 받는다.
- 고객으로부터 추가 구매 여부를 입력 받는다.
    - 추가 구매를 희망하면 처음으로 돌아가 반복한다.
    - 희망하지 않으면 프로그램을 종료한다.

포스기
- 날짜를 확인하여 현재 적용 가능한 프로모션을 확인한다.
...
- 프로모션은 N개 구매 시 1개 무료 증정(Buy N Get 1 Free)의 형태로 진행된다.
- 프로모션 혜택은 프로모션 재고 내에서만 적용할 수 있다.
- 프로모션 기간 중이라면 프로모션 재고를 우선적으로 차감하며, 프로모션 재고가 부족할 경우에는 정가 결제 여부를 확인하고 동의하면 일반 재고를 사용한다.
- 프로모션 적용이 가능한 상품에 대해 고객이 해당 수량보다 적게 가져온 경우, 필요한 수량을 추가로 가져오면 혜택을 받을 수 있음을 안내한다.
...
출력
- 재고 목록
  - 재고 파일에 상품의 프로모션 재고만 있으면, 일반 상품의 재고 없음을 알려야 함.
- 영수증